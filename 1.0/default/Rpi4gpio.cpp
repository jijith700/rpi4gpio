...
constexpr int kPinLed = 23;  // Number(gpio17)  = kDefaultGpioPortOffset + kPinLed = 458 + 17 = 475
constexpr int kPinSW = 24;   // Number(gpio18)  = kDefaultGpioPortOffset + kPinSW = 458 + 18 = 476

using android::hardware::automotive::vehicle::V2_0::VehiclePropValue;
using android::hardware::automotive::vehicle::V2_0::VehiclePropertyStatus;
using vendor::demo::vehiclevendor::V1_0::VehicleProperty;
using android::sp;
...
// Added manually
    Rpi4gpio::Rpi4gpio() {
        ...
        do {
            ...
            if (hGpioexport < 0) {
                ALOGE("Rpi4gpio Export Init Failed (%s)", gpio_export_path.str().c_str());
                valid = -1;
                break;
            }

            ALOGI("Rpi4gpio Export Init Successfully (%s)", gpio_export_path.str().c_str());
            ...
            if (ret < 0) {
                ALOGE("Rpi4gpio Led Port Init Failed");
                valid = -1;
                break;
            }

            ALOGI("Rpi4gpio Led Port Init Successfully");
            ...
            if (hGpioLedDir < 0) {
                ALOGE("Rpi4gpio Led direction Init Failed (%s)", gpio_led_direction.str().c_str());
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio Led direction Init Successfully (%s)",
                  gpio_led_direction.str().c_str());

            ret = write(hGpioLedDir, "out", strlen("out"));
            if (ret < 0) {
                ALOGE("Rpi4gpio Led Port direction Init Failed");
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio Led Port direction Init Successfully");
            ...
            if (ret < 0) {
                ALOGE("Rpi4gpio SW Port Init Failed");
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio SW Port Init Successfully");
            ...
            if (hGpioSWDir < 0) {
                ALOGE("Rpi4gpio SW direction Init Failed");
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio SW direction Init Successfully");

            ret = write(hGpioSWDir, "in", strlen("in"));
            if (ret < 0) {
                ALOGE("Rpi4gpio SW Port direction Init Failed");
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio SW Port direction Init Successfully");
            ...
            if (ret < 0) {
                ALOGE("Rpi4gpio SW Edge rising Init Failed");
                valid = -1;
                break;
            }
            ALOGI("Rpi4gpio SW Edge rising Init Successfully");
        ...
        if (valid >= 0) {
            ALOGI("Rpi4gpio Init Successfully, valid:%d", valid);
            //LedStatus status = get();
            //setVhal(static_cast<int>(status));
        } else {
            ALOGE("Rpi4gpio Init Failed, valid:%d", valid);
        }
    }
    ...
    Rpi4gpio::~Rpi4gpio() {
        ...
        do {
            if (1 == valid) {
                ...
                if (hGpiounexport < 0) {
                    ALOGE("Rpi4gpio Unexport Init Failed (%s)", gpio_unexport_path.str().c_str());
                    valid = -1;
                    break;
                }

                ALOGI("Rpi4gpio Unexport Init Successfully (%s)", gpio_unexport_path.str().c_str());
                ...
                if (ret < 0) {
                    ALOGE("Rpi4gpio Led Port Unexport Failed");
                    valid = -1;
                    break;
                }

                ALOGI("Rpi4gpio Led Port Unexport Successfully");
                ...
                if (ret < 0) {
                    ALOGE("Rpi4gpio SW Port Unexport Failed");
                    valid = -1;
                    break;
                }
                ALOGI("Rpi4gpio SW Port Unexport Successfully");
            }
        } while (0);
...
// Methods from ::vendor::demo::rpi4gpio::V1_0::IRpi4gpio follow.
    Return<::vendor::demo::rpi4gpio::V1_0::LedStatus> Rpi4gpio::get() {
        // TODO implement
        ...
        return state;
        //return ::android::hardware::Rpi4gpio::V2_0::LedStatus {};
    }

    Return<int32_t> Rpi4gpio::set(::vendor::demo::rpi4gpio::V1_0::LedStatus val) {
    ...
    void *setVhal(int status) {
        ALOGI("Rpi4gpio setVhal status= %d", status);
        sp<IVehicle> iVehicleService = IVehicle::getService();
        if (iVehicleService != nullptr) {
            ALOGE("Rpi4gpio IVehicle service updating hal");
            VehiclePropValue propValue{
                    .timestamp = android::elapsedRealtimeNano(),
                    .areaId = 0,
                    .prop = static_cast<int32_t>(VehicleProperty::RPI4_GPIO),
                    .status = VehiclePropertyStatus::AVAILABLE,
                    .value.int32Values{status}};
            iVehicleService->set(propValue);
        }
        return nullptr;
    }
    ...
    Return<void>Rpi4gpio::on() {
        ...
        if (hGpioLed < 0) {
            ALOGE("Rpi4gpio %s is unavailable!", gpio_led_value.str().c_str());
            return Void();
        }
        ...
        if (ret < 0) {
            ALOGE("Rpi4gpio LED Port write on Failed");
        ...
        //state = LedStatus::LED_ON;
        ALOGI("Rpi4gpio On, status:%d", state);
        return Void();
    }
    ...
    Return<void> Rpi4gpio::off() {
        ...
        if (valid < 0) {
            ALOGE("Rpi4gpio is unavailable! valid=%d", valid);
            return Void();
        }
        ...
        if (ret < 0) {
            ALOGE("Rpi4gpio LED Port write off Failed");
            //state = LedStatus::LED_BAD_VALUE;
        }
        ...
        //state = LedStatus::LED_OFF;
        ALOGI("Rpi4gpio Off, status:%d", state);
        return Void();
    }
    ...
    void *work(void *param) {
        ...
        do {   // exit by error
            ...
            pParent = (Rpi4gpio *) param;
            ALOGD("Created thread!");

            gpio_sw_value << kGPIOSysfsPath << "/gpio" << (kDefaultGpioPortOffset + kPinSW)
                          << "/value";
            hGpioSWvalue = open(gpio_sw_value.str().c_str(), O_RDONLY);
            if (hGpioSWvalue < 0) {
                ALOGE("Rpi4gpio SW Value Init Failed");
                break;
            }
            ...
            while (!destroyThread) {
                ...
                for (int n = 0; n < swEvents; ++n) {
                    if (events[n].data.fd) {  // == hGpioSWvalue?
                        ...
                        //Unblock Return<LedStatus> Rpi4gpio::get()
    ...
    Return<void>
    Rpi4gpio::registerCallback(const sp<::vendor::demo::rpi4gpio::V1_0::IRpi4gpioCallback> &cb) {
